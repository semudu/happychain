class SQL:
    ...
    # --------------- TRANSACTION --------------- #
    ADD_TRANSACTION = "insert into transaction (sender_id, receiver_id, amount, date, message_id, content, is_active, type) values (%s,%s,%s,CURRENT_TIMESTAMP(),%s,%s,1,%s);"
    ...
    GET_USER_SENT_COUNT_BY_RECEIVER_TODAY = "select count(*) from transaction where sender_id = %s and receiver_id = %s and date(date) = curdate() and is_active = 1;"
    GET_USER_SENT_COUNT_BY_RECEIVER_TEAM_TODAY = "select count(*) from transaction t, user u where t.sender_id = u.id and date(t.date) = curdate() and t.is_active = 1 and t.sender_id = %s and u.team_id = (select team_id from user where id = %s);"
    GET_LAST_N_SENT = "select concat(u.first_name, ' ', u.last_name) as full_name, (case when m.id = -1 then (select content ->> '$.content' from transaction where id = t.id) else m.text end) text, date_format(t.date, '%d %M, %W') date from transaction t, user u, message m where t.receiver_id = u.id and t.message_id = m.id and t.is_active = 1 and t.sender_id = %s and (t.id != -1 or (t.id = -1 and t.content is not null )) order by t.date desc limit %s;"
    GET_LAST_N_RECEIVED = "select concat(u.first_name, ' ', u.last_name) as full_name, (case when m.id = -1 then (select content ->> '$.content' from transaction where id = t.id) else m.text end) text, date_format(t.date, '%d %M, %W') date from transaction t, user u, message m where t.sender_id = u.id and t.message_id = m.id and t.is_active = 1 and t.receiver_id = %s and (t.id != -1 or (t.id = -1 and t.content is not null )) order by t.date desc limit %s;"
    GET_LAST_N_RECEIVED_MESSAGES_BY_TEAM = "select u_s.first_name from_first_name, u_s.last_name from_last_name, tm.name from_team, u_r.first_name to_first_name, u_r.last_name to_last_name, (case when t.message_id = -1 then t.content ->> '$.content' else m.text end) message, unix_timestamp(t.date) 'timestamp' from transaction t, team tm, user u_r, user u_s, message m where u_s.id = t.sender_id and u_s.team_id = tm.id and t.receiver_id = u_r.id and m.id = t.message_id and u_r.team_id = %s order by t.date desc limit %s;"
    ARCHIVE_ACTIVE_TRANSACTIONS = "insert into transaction_archive (date, archive_date, sender_id, receiver_id, message_id, content, amount, type) select date, curdate(), sender_id, receiver_id, message_id, content, amount, type from transaction where is_active = 1;"
    ...
    GET_SHARE_MESSAGE_LIST_BY_SCOPE_ID = "select distinct m.* from message m, team t where m.type = 'P' and m.direction = 'IN' and t.scope_id = %s and ((exists(select * from message where scope_id = t.scope_id) and m.scope_id = t.scope_id) or (not exists(select * from message where scope_id = t.scope_id) and m.scope_id = 0))"
    ...
    GET_MESSAGES_BY_SCOPE_ID = "select case when message_id = -1 then t.content ->> '$.content' else m.text end Mesaj, count(1) \"Gönderim Sayısı\" from transaction t, message m where t.message_id = m.id and t.is_active = 1 and sender_id in (select u.id from user u, team t where u.team_id = t.id and t.scope_id = %s) group by Mesaj order by count(1) desc"
